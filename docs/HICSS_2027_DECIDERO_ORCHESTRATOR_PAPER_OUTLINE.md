...
## ✍️ Length Budget and Figure Plan (drafting guide)

**Hard limit: 10 pages (HICSS format, incl. references). Target body ≈ 6,500 words.**
Draft each section to the word target below, then trim to fit. At HICSS two-column
density (~750 words/page) 6,500 words ≈ 8.5 pages of prose, leaving ~1.5 pages for
figures + references. Keep **4 core figures + 1 table**; treat the rest as optional
and cut first if over length. Every figure should be *generated from the real
artifact* where possible (the AST→diagram exporter, the report renderer) so it
cannot drift from what the system does.

### Word budget by section

| Section | Target words | Figures / Tables |
|---|---:|---|
| Abstract (not in body count) | ~150 | — |
| 1. Introduction | 650 | — |
| 2. Background and Motivation | 750 | — |
| 3. Design Objectives | 400 | **T1** (objectives → mechanism → evidence) |
| 4. System Architecture | 1,400 | **F1** (anchor), **F2** (layers), F5 (opt.) |
| 5. Reference Method: Delphi | 1,150 | **F3** (gate moment), F4 (convergence) |
| 6. Evidence and Validation | 1,000 | F4 may live here instead; reuse T1 |
| 7. Discussion | 650 | — |
| 8. Conclusion | 300 | — |
| **Total (body)** | **≈ 6,300** | 4 core figures + 1 table (+2 optional) |

~200-word headroom against the 6,500 target absorbs transitions and figure captions.

### Figure catalog

- **F1 — Composition = recursion (ANCHOR FIGURE).** Two panels: *left*, a trimmed
  `orchestrations/delphi.json` (the nested `iterate → sequence → [rank, comment]`
  block); *right*, the same structure as nested containment boxes (the iterate is a
  box with a dashed loop-back arrow, containing the round-subcycle box, containing
  the activity tiles), color-keyed node↔JSON block. The one figure that carries the
  thesis: *the method is a tree in the data, and the engine walks it.* Generate with
  `scripts/export_orchestration_diagram.py` (Mermaid/Graphviz) so it tracks the real
  document. Supports Contributions 1–2. **§4 (or open §5).**
- **F2 — The four layers.** A vertical stack: Activities (frozen bricks) →
  Orchestration document (method-as-data) → Engine + shared primitives
  (predicate/transform/summarizer/recommender registries) → Bundles (typed
  handoff). Annotate that *method logic lives only in the Layer-2 document*. Simple
  hand-drawn/vector diagram. Supports Contributions 1–2 and the "no special-purpose
  Delphi activity" claim. **§4.**
- **F3 — The decision moment (AI proposes, facilitator disposes).** A facilitator
  round-gate screenshot/mock: round evidence (round N of M, agreement state), the
  recommended option with its **source** (computational rule *or* AI advisor) and
  the AI's one-line rationale, and the continue/conclude controls. Carries
  Contribution 3 and the "one slot, two backends" recommendation duality. Capture
  from the running gate panel. **§5.**
- **F4 — Convergence over rounds (the report tells the consensus story).** A chart
  generated by `report_renderers.py` from a run: Kendall's W (and/or median-IQR)
  rising/falling across rounds, optionally beside the rank-trajectory bump chart.
  Shows the method actually executes *and* that outputs are structured, derived
  artifacts (§M/§N). Use the synthetic fixture so it's reproducible. **§5 or §6.**
- **F5 — (optional) Static document → unfolding agenda.** The stack machine
  materializing round 1 then round 2: the fixed document on the left, the
  round-by-round agenda it produces on the right ("where it started / is now / is
  going"). Cut first if tight. **§4.**
- **T1 — Objectives → mechanism → evidence (TABLE).** Three columns mapping each
  Design Objective (§3) to the architectural mechanism that meets it and the
  implemented evidence item (§6) that demonstrates it. High density-to-space; lets
  §3/§6 stay short. **§3 (referenced again in §6).**
- **T2 — (optional) The small grammar.** Step types (sequence/iterate/activity/
  facilitator-decision/ai-decision) + the four primitive registries, one line each.
  Only if space remains. **§4.**

## Proposed Paper Structure

### 1. Introduction
**[Target: 650 words · Figures: none]**
...
### 3. Design Objectives
**[Target: 400 words · Table T1 (objectives → mechanism → evidence) — lets this section stay terse by offloading the mapping to the table]**
...
**[Target: 1,400 words (the core; spend the budget here) · Figures: F1 anchor (composition=recursion), F2 (four layers); F5 + T2 optional]**
...
**[Target: 1,150 words · Figures: F3 (the decision moment — AI proposes/facilitator disposes), F4 (convergence over rounds; may move to §6)]**
...
**[Target: 1,000 words · Reuse T1 (cite the evidence column); F4 may live here. Compress the implemented-evidence list into prose grouped by claim — do NOT enumerate all 14 items at full length in the paper.]**
...
### 7. Discussion
**[Target: 650 words · Figures: none. Fold Future Directions (recursion theory, DAG static-vs-dynamic safety, thinkLet composer, AI co-author) in here compactly; the single-threaded-driver tradeoff is the strongest standalone point.]**
