...
def test_brainstorming_returns_empty_list_for_activity_with_no_ideas(
    authenticated_client: TestClient,
    user_manager_with_admin: UserManager,
    db_session,
):
    ...
    meeting = meeting_manager.create_meeting(
        meeting_data=MeetingCreate(
            title="Empty Brainstorm Test",
            description="Testing empty brainstorm behavior.",
            start_time=start_time,
            end_time=start_time + timedelta(minutes=30),
            duration_minutes=30,
            publicity=PublicityType.PRIVATE,
    ...
    try:
        ...
        # GET ideas list directly
        response = authenticated_client.get(
            f"/api/meetings/{meeting.meeting_id}/brainstorming/ideas?activity_id={activity_id}"
        )
