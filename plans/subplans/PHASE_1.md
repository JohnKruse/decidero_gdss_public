...
### Step 1 — [DONE] Author the JSON Schemas and Wire Startup Validation
...
- `jsonschema` is not present in the locked runtime dependencies, so this step added `app/services/contract_schemas.py` as a small local validator for the schema subset enforced at startup and in tests rather than adding a new dependency under restricted network conditions.
- Runtime bundle writes were not globally rejected on missing item-level provenance in this step; the bundle schema and validator are invokable for tests and future engine work, while existing legacy bundle producers remain compatible until later hardening steps decide whether to normalize or reject those payloads.

### Step 2 — [DONE] Split the Contract Document and Name the Design Principles
...
Technical deviations logged:
- DP3 and DP6 rows in the DP-to-test mapping table intentionally point to future `app/tests/test_activity_plugins.py` coverage because Phase 1 Step 3 owns their executable enforcement.
- DP5 cites the manifest schema test as the current structured-metadata witness and records the Step 4 audit conformance test as pending, because the ThinkLet audit document is not authored until Step 4.

### Step 3 — [DONE] Close the DP3 Idempotency Gap and Resolve DP6 Disposition
