# PHASE 4 [COMPLETE] — Engine and Step Kinds
...
### Step 4 — [DONE] Implement the `facilitator-decision` Step Kind
...
Technical deviations logged:
- The pause is represented by an `AgendaActivity` row with `tool_type="facilitator_decision"` (exposed as `OrchestrationEngineStrategy.FACILITATOR_DECISION_TOOL_TYPE`) rather than a parallel pseudo-row table. `ActivityBundle.activity_id` is NOT NULL in the model, so writing the decision's captured-choice bundle requires an activity row to attach it to; using an `AgendaActivity` keeps the bundle stream uniform and lets the Phase 1 schema validate the captured item unchanged.
- The pause is observable via `pending_decision()` / `is_paused()` rather than by changing the `AgendaStrategy` interface. The existing `create_activity` contract returns the placeholder activity once, then raises `HTTPException(409, ...)` until `resume_with_facilitator_decision` is called. Adding a new abstract method to the strategy base would have rippled into `LinearAgendaStrategy` without earning any benefit there, so the pause API is additive on the orchestration strategy only.

### Step 5 — [DONE] Implement the `ai-decision` Step Kind
...
Technical deviations logged:
- The Phase 3 reliability execution analogue is `app/services/reliable_writes.py::run_with_retry`, not `reliability_rehearsal.py` (which is purely an analysis/gating module). The ai-decision dispatch reuses `run_with_retry` verbatim with a custom `should_retry_result` callback that treats schema-validation failures as retryable, satisfying the spec's "canonical reuse case" requirement without altering the retry path.
- The AI provider in `app/services/ai_provider.py` is async; the engine is sync. Rather than restructure the engine, the strategy accepts an injectable sync `ai_caller(prompt, settings) -> str` (with a default that wraps `chat_complete` via `asyncio.run`). Tests inject a synchronous stub so they never touch the network. `ai_settings` and `ai_retry_policy` are also injectable on the strategy constructor.
- A minimal in-module JSON-Schema-style validator (`_validate_output_schema`) was authored rather than introducing a new `jsonschema` dependency, matching the Step 1 zero-dependency strategy for orchestration document validation. The validator supports `type` / `required` / `properties` / `items` — the subset the ai-decision tests exercise — and intentionally rejects `bool` for `integer`/`number` types.
